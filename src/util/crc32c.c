...
static const uint32_t rdb_crc32c_table[256] = {
...
static const uint32_t rdb_crc32c_mask_delta = 0xa282ead8;
...
rdb_crc32c_mask(uint32_t crc) {
  ...
  return ((crc >> 15) | (crc << 17)) + rdb_crc32c_mask_delta;
}

uint32_t
rdb_crc32c_unmask(uint32_t masked_crc) {
  uint32_t rot = masked_crc - rdb_crc32c_mask_delta;
