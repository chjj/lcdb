...
ldb_txn_create(ldb_t *db, const ldb_snapshot_t *snapshot) {
  ...
  if (snapshot != NULL) {
    ...
    txn->sta = ldb_staging_create(db->versions);

    ldb_mutex_init(&txn->mutex);
    ldb_memtable_ref(txn->mem);
    ldb_staging_ref(txn->sta);
...
ldb_txn_cleanup(ldb_txn_t *txn) {
  ...
  for (i = txn->sta->files.length - 1; i != (size_t)-1; i--) {
    ldb_filemeta_t *file = txn->sta->files.items[i];
...
ldb_txn_open(ldb_t *db, int flags, ldb_txn_t **txn) {
  ...
  if (flags & LDB_RDONLY) {
    *txn = ldb_txn_create(db, ldb_snapshot(db));
    ...
    if (rc == LDB_OK) {
      while (db->txn != NULL || db->writers.length > 0)
        ldb_cond_wait(&db->writable_signal, &db->mutex);
...
ldb_txn_add_files(ldb_txn_t *txn, rb_set64_t *live) {
  ...
  for (i = 0; i < txn->sta->files.length; i++) {
    ldb_filemeta_t *file = txn->sta->files.items[i];
...
ldb_txn_flush(ldb_txn_t *txn, ldb_mutex_t *mu) {
  ...
  if (rc == LDB_OK) {
    ...
    ldb_staging_t *sta = ldb_staging_clone(txn->sta);
    ...
    ldb_staging_push(txn->sta, &entry->meta);
  }
...
ldb_txn_get(ldb_txn_t *txn, const ldb_slice_t *key,
  ...
  if (ldb_memtable_get(mem, &lkey, value, &rc)) {
    /* Done. */
  } else if (ldb_staging_get(sta, options, &lkey, value, &rc)) {
    /* Done. */
  } else {
...
ldb_txn_commit(ldb_txn_t *txn, const ldb_writeopt_t *options) {
  ...
  if (txn->sta->files.length == 0 && usage > 0) {
    ldb_batch_t batch;
    ...
    if (rc == LDB_OK && txn->sta->files.length > 0) {
...
ldb_txn_iterator(ldb_txn_t *txn, const ldb_readopt_t *options) {
  const ldb_comparator_t *ucmp = txn->comparator->user_comparator;
  ldb_seqnum_t seq, ignore;
  ldb_t *db = txn->db;
  ldb_memtable_t *mem;
  ldb_vector_t list;
  ldb_iter_t *iter;
  ...
  if (options->snapshot != NULL)
    return ldb_emptyiter_create(LDB_INVALID);

  if (txn->snapshot != NULL) {
    ldb_readopt_t opt = *options;
    opt.snapshot = txn->snapshot;
    return ldb_iterator(db, &opt);
  }
  ...
  ldb_staging_add_iterators(txn->sta, options, &list);
  ...
  return ldb_dbiter_create(db, ucmp, iter, seq, seed);
}
